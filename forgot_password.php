<?php
// D:\Xampp\htdocs\FP\forgot_password.php
// This page allows the user to request a password reset link.
include 'db_connect.php'; 
include 'get_stats.php'; // Include stats for consistent sidebar design
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Campus Lost & Found - Reset Password</title>
  ...
  <style>
      ...
      #resetMessage { color: green; text-align: center; margin-top: 10px; } 
  </style>
</head>
<body>
  <div class="min-h-screen flex flex-col md:flex-row">
    ...
    <div class="flex-1 flex justify-center items-center bg-gray-50 p-8">
      <div class="form-container animate-fade-in p-8 w-full max-w-md bg-white rounded-xl shadow-2xl text-gray-800">
        <h2 class="text-2xl font-bold mb-6 text-center heading-accent">Reset Password</h2>
        <p class="text-center text-gray-600 mb-6">Enter your registered email to receive a password reset link.</p>

        <form id="resetForm" class="space-y-6">
          <div class="input-group">
            <input type="email" id="email" name="email" required class="input-field peer" placeholder=" " />
  ...
  <script>
    document.getElementById("resetForm").addEventListener("submit", async function(e) {
      ...
      try {
        // You MUST implement the 'forgot_password' action in your api.php
        const response = await fetch('/FP/api.php?action=forgot_password', { 
          method: 'POST',
        ...
        if (response.ok && result.success) {
          ...
          // Important: Always display a generic success message to prevent email enumeration attacks.
          messageElement.textContent = "If this email is registered, you will receive a password reset link shortly.";
        } else {
          messageElement.style.color = "red";
          messageElement.textContent = "Error: Could not process request. Please check your email and try again.";
        }
      } catch (networkError) {
        messageElement.style.color = "red";
